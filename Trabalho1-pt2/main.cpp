...
bool mostraParede = false;
bool mostraSombra = true;
bool pontual = true;
...
void desenha() {
    ...
    glTranslated(0, 0, -8);
    GUI::setLight(1, 0.5, 5, 1.5, true, false, false, false, true);
    ...
    if( mostraParede ){
        int heightLat = 5;
        int widthLat = 18;
        glPushMatrix();         //parede-lateral 1
            glTranslated(-6, 2.48, 0);
            glRotated(-90, 0, 0, 1);
            GUI::setColor(0.8, 0.8, 0.9);
            GUI::drawFloor(heightLat, widthLat);
        glPopMatrix();
        glPushMatrix();          //parede-backg
            glTranslated(0, 2.48, -9);
            glRotated(90, 1, 0, 0);
            GUI::setColor(0.8, 0.8, 0.9);
            GUI::drawFloor(12, 5);
        glPopMatrix();
        glPushMatrix();           //parede-lateral 2
            glTranslated(6, 2.48, 0);
            glRotated(90, 0, 0, 1);
            GUI::setColor(0.8, 0.8, 0.9);
            GUI::drawFloor(heightLat, widthLat);
    ...
    //----Início "Sombreamento"----
    GLfloat sombras [4][4];
    float posIlumin[4] = {glutGUI::lx, glutGUI::ly+4, glutGUI::lz, pontual};

    if( mostraSombra && glutGUI::enabled_light[7] ){

        glColor3d(0.2, 0.2, 0.2);

        bool auxiAxis = glutGUI::draw_eixos;
        glutGUI::draw_eixos = false;
        glDisable(GL_LIGHTING);

        for (int i = 0; i < objetosVector.size(); i++){

            glPushMatrix();                     //Projetando sombras no piso
                glTranslatef(0, 0.001, 0);
                GUI::shadowMatrixYk( sombras, posIlumin, k );
                glMultTransposeMatrixf( (GLfloat*)sombras );
                if( objetosVector[i]->sombra ){
...
void teclado(unsigned char key, int x, int y) {
    ...
    switch ( key ){
        ...
        //Controle do Arquivo com as configs
        case 'S':{
            saveBaseFile();
            break;
        }

        //Comandos p/ inclusão de objetos no cernário;

        case 'L':{
            mostraParede = !mostraParede;
            break;
        }
        case 'Ç':{
            mostraSombra = !mostraSombra;
            break;
        }
        case '!':{
            if ( incluirObjeto ){
        ...
        glutGUI::cam = new CameraJogo(0.2,4,-17,  0,0,2.7,  0,1,0); //-10,10,-20,  2,0,0,  0,1,0
                    //new Camera( );
                    //new CameraJogo(0.2,4,-17,  0,0,2.7,  0,1,0); //-10,10,-20,  2,0,0,  0,1,0
            break;
        }
        case '4':{
            delete glutGUI::cam;
...
int main(){
    ...
    cout << "  Desenhar paredes em torno do cenário: ( L + SHIFT )" <<endl;
    cout << "  Habilitar sombras no cenário: ( Ç + SHIFT )" <<endl << endl;
    cout << "  Incluir novo objeto no cenário: ( + )" << endl;
    cout << "  A lista de objetos vai de 1 a 0 (não tec numérico) + SHIFT" << endl << endl;
    ...
    cout << "Retornar para câmera dafult: ( c )" <<endl;

    cout << endl;
    carregarArquivo();
    //pocisionamento da Câmera defualt;
    //glutGUI::cam = new CameraDistante(10,1,5, 8,-5,-1, 0,1,0);
